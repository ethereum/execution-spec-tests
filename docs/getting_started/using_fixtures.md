...
1. The `ethereum/pyspec` simulator executes the test cases against a fully instantiated execution client that receives blocks via the [Engine JSON-RPC API](https://github.com/ethereum/execution-apis/blob/main/src/engine/README.md). Each block is verified by the client and tested via the Engine API response. The post-state section of the test is additionally explicitly verified via the Engine API (post-Merge forks only).
