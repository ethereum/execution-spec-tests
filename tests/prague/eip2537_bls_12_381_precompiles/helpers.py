...
class BLSPointGenerator:
    ...
    def generate_g1_map_isogeny_kernel_points(cls) -> List[FP]:
        ...
        generate these points: ```sage q =
        0x1A0111EA397FE69A4B1BA7B6434BACD764774B84F38512BF6730D2A0F6B0F6241EABFFFEB153F
        FFFB9FEFFFFFFFFAAAB Fq = GF(q) E1 = EllipticCurve(Fq, (0, 4)) #
        BLS12-381 curve ISO_11_A =
        Fq(0x144698A3B8E9433D693A02C96D4982B0EA985383EE66A8D8E8981AEFD881AC98936F8DA0E0
        F97F5CF428082D584C1D) ISO_11_B =
        Fq(0x12E2908D11688030018B12E8753EEE3B2016C1F0F24F4070A0B9C14FCEF35EF55A23215A31
        6CEAA5D1CC48E98E172BE0) ISO_11_Z = Fq(11) Ei = EllipticCurve(Fq,
        (ISO_11_A, ISO_11_B)) iso = EllipticCurveIsogeny(E=E1, kernel=None,
        codomain=Ei, degree=11).dual() for (x, _) in
        iso.kernel_polynomial().roots(): discriminant = 1 - 4 / (ISO_11_A /
        ISO_11_B * x + 1) if not discriminant.is_square(): continue for sign in
        [1, -1]: zt2 = (-1 + sign * discriminant.sqrt()) / 2 t2 = zt2 /
        ISO_11_Z if t2.is_square(): t = t2.sqrt() assert x == -ISO_11_B /
        ISO_11_A * (1 + 1 / (ISO_11_Z**2 * t**4 + ISO_11_Z * t**2)) print(t)
        ```
        ...
        The roots and their properties are as follows: - Root 1
        (x=3447232547282837364692125741875673748077489238391001187748258124039623697289
        612052402753422028380156396811587142615): Fails because its
        discriminant is not a square. - Root 2
        (x=3086251397349454634226049654186198282625136597600255705376316455943570106637
        401671127489553534256598630507009270951): Fails because its
        discriminant is not a square. - Root 3
        (x=2512099095366387796245759085729510986367032014959769672734622752070562589059
        815523018960565849753051338812932816014): Has a square discriminant,
        but both sign options yield t^2 values that are not squares. - Root 4
        (x=2077344747421819657086473418925078480898358265217674456436079722467637536216
        749299440611432676849905020722484031356): Yields two valid t values: -
        1731081574565817469321317449275278355306982786154072576198758675751495027640363
        897075486577327802192163339186341827 -
        8614106910527620883007905873948100743035058966280483055356452849221351166767559
        56131724844456716837983264353875219 - Root 5
        (x=1629023065307570116876483814580399609058797608540074345321518038064223832399
        05014872915974221245198317567396330740): Yields two valid t values: -
        1006044755431560595281793557931171729984964515682961911911398807521437683216171
        091013202870577238485832047490326971 -
        1562001338336877267717400325455189014780228097985596277514975439801739125527323
        838522116502949589758528550231396418
