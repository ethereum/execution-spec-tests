...
@pytest.mark.parametrize(
    "code_a,code_b,tx_data,code_address_storage",
    [
        (
            (
                Op.MSTORE8(offset=0x0, value=0x25)
                + Op.MSTORE8(offset=0x1, value=0x60)
                + Op.CALL(
                    gas=0xFFFFFF,
                    address=0x200,
                    value=0x0,
            ...
            (Op.PUSH1[0x0] + Op.CALLDATALOAD + Op.PUSH1[0x0] + Op.SSTORE + Op.STOP),
            b"\x00",
            Account(
...
def test_calldataload(
    ...
    """
    code_a_address = pre.deploy_contract(code_a)
    code_b_address = pre.deploy_contract(code_b)
