...
from ethereum_test_forks import Byzantium, Cancun, Constantinople, Istanbul, London
...
program_blockhash = pytest.param(
    Op.MSTORE(32, Op.BLOCKHASH(0)) + Op.RETURN(0, 32),
    ProgramResult(result=ScenarioExpectOpcode.BLOCKHASH_0),
...
program_difficulty_randao = pytest.param(
    Op.MSTORE(32, Op.PREVRANDAO) + Op.RETURN(0, 32),
    ProgramResult(result=ScenarioExpectOpcode.DIFFICULTY_RANDAO),
...
program_basefee = pytest.param(
    Op.MSTORE(32, Op.BASEFEE) + Op.RETURN(0, 32),
    ProgramResult(result=ScenarioExpectOpcode.BASEFEE, from_fork=London),
    id="program_BASEFEE",
)

program_blobhash = pytest.param(
    Op.MSTORE(0, Op.BLOBHASH) + Op.RETURN(0, 32),
    ProgramResult(result=ScenarioExpectOpcode.BLOBHASH_0, from_fork=Cancun),
    id="program_BLOBHASH",
)

program_blobbasefee = pytest.param(
    Op.MSTORE(32, Op.BLOBBASEFEE) + Op.RETURN(0, 32),
    ProgramResult(result=ScenarioExpectOpcode.BLOBBASEFEE, from_fork=Cancun),
