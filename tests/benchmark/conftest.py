...
def pytest_collection_modifyitems(config, items):
    ...
    bloatnet_marker = pytest.mark.bloatnet
    ...
    run_bloatnet = marker_expr and "bloatnet" in marker_expr and "not bloatnet" not in marker_expr
    ...
    for i, item in enumerate(items):
        ...
        # Add markers to tests in benchmark directory
        if is_in_benchmark_dir:
            if not item.get_closest_marker("benchmark") and not item.get_closest_marker(
                "bloatnet"
            ):
        ...
        # Add bloatnet marker to bloatnet tests
        if is_bloatnet_test and not item.get_closest_marker("bloatnet"):
            item.add_marker(bloatnet_marker)

        # Remove tests based on marker selection
        if is_bloatnet_test:
            if not run_bloatnet:
                items_for_removal.append(i)
        elif is_benchmark_test:
            if not run_benchmarks:
                items_for_removal.append(i)
        elif run_benchmarks or run_bloatnet:
            items_for_removal.append(i)
