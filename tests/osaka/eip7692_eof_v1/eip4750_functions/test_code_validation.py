...
def test_callf_stack_height_limit_exceeded(eof_test, callee_outputs):
    ...
    callf_stack_height = 1024 - callee_outputs
    container = Container(
        sections=[
            Section.Code(
                Op.PUSH0 * callf_stack_height + Op.CALLF[1] + Op.POP + Op.STOP,
...
@pytest.mark.parametrize("callee_outputs", [1, 2, MAX_CODE_OUTPUTS])
def test_callf_stack_overflow_by_outputs(eof_test, callee_outputs):
    """
    Test for invalid EOF code containing CALLF instruction exceeding the runtime stack height limit
    by calling a function with non-zero outputs.
    """
    callf_stack_height = 1025 - callee_outputs
    container = Container(
        sections=[
            Section.Code(
                Op.PUSH0 * callf_stack_height + Op.CALLF[1] + Op.STOP,
                max_stack_height=MAX_OPERAND_STACK_HEIGHT,  # this value is invalid
            ),
            Section.Code(
...
@pytest.mark.parametrize("callee_stack_height", [2, 3, MAX_OPERAND_STACK_HEIGHT])
def test_callf_stack_overflow_by_height(eof_test, callee_stack_height):
    """
    Test for invalid EOF code containing CALLF instruction exceeding the runtime stack height limit
    by calling a function with non-zero max stack height.
    """
    container = Container(
        sections=[
            Section.Code(
