...
def test_callf_stack_overflow(eof_test: EOFTestFiller, stack_height: int):
    """Test CALLF instruction calling same function causing stack overflow at validation time."""
    container = Container(
        sections=[
            Section.Code(code=Op.CALLF[1] + Op.STOP),
            Section.Code(
...
def test_callf_stack_overflow_after_callf(eof_test: EOFTestFiller, stack_height: int):
    """Test CALLF intruction calling next function causing stack overflow at validation time."""
    container = Container(
        sections=[
            Section.Code(code=Op.CALLF[1] + Op.STOP),
            Section.Code(
...
def test_callf_stack_overflow_variable_stack_2(eof_test: EOFTestFiller, stack_height: int):
    """Test CALLF instruction causing stack overflow."""
    container = Container(
        name="callf_stack_overflow_variable_stack_3",
        sections=[
            Section.Code(
                code=Op.PUSH0
                + Op.PUSH1[0]
