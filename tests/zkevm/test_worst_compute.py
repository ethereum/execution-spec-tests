...
def test_reference_jumps(
    ...
    gsc = fork.gas_costs()
    # Gas cost breakdown for JUMP(ADD(5, PC)) + JUMPDEST:
    # - PC: G_BASE (2) - pushes program counter to stack
    # - PUSH1(5): G_VERY_LOW (3) - pushes constant 5 to stack
    # - ADD: G_VERY_LOW (3) - adds PC and 5
    # - JUMP: G_MID (8) - jumps to calculated address
    # - JUMPDEST: G_JUMPDEST (1) - marks valid jump destination
    # Total: 2 + 3 + 3 + 8 + 1 = 17
    iteration_gas_cost = gsc.G_BASE + gsc.G_VERY_LOW * 2 + gsc.G_MID + gsc.G_JUMPDEST

    # Bytes per iteration calculation:
    # - PC: 1 byte (opcode)
    # - PUSH1(5): 2 bytes (opcode + value)
    # - ADD: 1 byte (opcode)
    # - JUMP: 1 byte (opcode)
    # - JUMPDEST: 1 byte (opcode)
    # Total: 6 bytes per iteration
    bytes_per_iter = 6
    ...
    # Create code with repeated pattern
    code_pattern = Op.JUMP(Op.ADD(5, Op.PC)) + Op.JUMPDEST
    # Convert to bytes to ensure it can be properly handled by len() and other functions
    code = bytes(code_pattern * (MAX_CODE_SIZE // 6))
    ...
    gas_values = f"{gas_used}/{available_gas}"
    print(
        f"{'REFERENCE JUMPS':<30} {bytecode_values:>15} ({code_size_pct:6.2f}%)  {gas_values:>20} ({gas_pct:6.2f}%)  {bytes_per_iter:>4}  {num_iters:>10}"
    )
