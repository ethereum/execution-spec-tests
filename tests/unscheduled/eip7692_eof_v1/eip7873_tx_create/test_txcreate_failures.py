...
def test_eof_txcreate_msg_depth(
    ...
    """
    Test TXCREATE handles msg depth limit correctly (1024). NOTE: due to block
    ...
    that test is that here calls and `TXCREATE`s alternate in order to reach
    the max depth. `who_fails` decides whether the failing depth 1024 will be
    on a call or on an `TXCREATE` to happen.
    """  # noqa: E501
