...
def test_eof_eofcreate_msg_depth(
    ...
    """
    Test EOFCREATE handles msg depth limit correctly (1024). NOTE: due to block
    gas limit and the 63/64th rule this limit is unlikely to be hit on mainnet.
    NOTE: See
    `tests/unscheduled/eip7692_eof_v1/eip7069_extcall/test_calls.py::test_eof_calls
    _msg_depth` for more explanations and comments. Most notable deviation from
    that test is that here calls and `EOFCREATE`s alternate in order to reach
    the max depth. `who_fails` decides whether the failing depth 1024 will be
    on a call or on an `EOFCREATE` to happen.
    """  # noqa: E501
...
def test_reentrant_eofcreate(
    ...
    # from the outer EOFCREATE. Storage in 1 should have 0 from the inner
    # EOFCREATE. For the created contract storage in `slot_counter` should be 1
