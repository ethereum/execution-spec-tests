...
def test_callf_stack_height_limit_exceeded(eof_test, callee_outputs):
    ...
    The code reaches the maximum runtime stack height (1024) which is above the
    EOF limit for the stack height in the type section (1023).
    """
...
def test_callf_stack_overflow_by_height(eof_test, callee_stack_height):
    ...
    The callee with the maximum stack height of 1 is valid because runtime
    limit (1024) is 1 bigger than the EOF limit (1023).
    """
    container = Container(
        sections=[
            Section.Code(
