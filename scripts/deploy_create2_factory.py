...
def deploy_factory(rpc_url: str):
    ...
    factory_bytecode = bytes([
        # Runtime code
        0x60, 0x00,  # PUSH1 0x00 (value for CREATE2)
        0x60, 0x00,  # PUSH1 0x00 (salt position in calldata)
        0x35,  # CALLDATALOAD (load salt)
        0x60, 0x20,  # PUSH1 0x20 (bytecode starts at position 32)
        0x36,  # CALLDATASIZE
        0x60, 0x20,  # PUSH1 0x20
        0x03,  # SUB (bytecode length = calldatasize - 32)
        0x80,  # DUP1 (duplicate bytecode length)
        0x60, 0x20,  # PUSH1 0x20 (source position in calldata)
        0x60, 0x00,  # PUSH1 0x00 (dest position in memory)
