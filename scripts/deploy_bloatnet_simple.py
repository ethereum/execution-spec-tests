...
def deploy_contracts(rpc_url: str, num_contracts: int, factory_address: str = None):
        ...
        if len(factory_code) > 0:
            ...
            try:
                result = subprocess.run(
                    [sys.executable, "scripts/deploy_create2_factory.py", "--rpc-url", rpc_url],
                    capture_output=True,
                    text=True
                )
    ...
    # Push bytecode size, push code offset, push memory offset, CODECOPY, push size, push offset, RETURN
    bytecode_size = 24576
    init_size = 13  # Size of init code instructions

    # PUSH2 bytecode_size, PUSH1 init_size, PUSH1 0, CODECOPY
    init_code.extend([
        0x61, (bytecode_size >> 8) & 0xFF, bytecode_size & 0xFF,  # PUSH2 bytecode_size
        0x60, init_size,  # PUSH1 init_size (offset where runtime code starts)
...
def main():
    ...
    parser.add_argument(
        "--factory-address",
        default=None,
        help="CREATE2 factory address (deploys new one if not provided or if address doesn't have code)"
    )
