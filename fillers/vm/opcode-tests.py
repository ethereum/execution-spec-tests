...
opcodes = [
    ...
    Opcode("sload", "54", 1, 2100),  # add 2000 for the cost of accessing a cold storage
    Opcode("sstore", "55", 2, 2200), # add 2000 for the cost of accessing a cold storage
    # JUMP and JUMPI are special cases, because we can't just run them
    Opcode("pc", "58", 0, 2),
    Opcode("msize", "59", 0, 2),
    Opcode("gas", "5a", 0, 2),
    Opcode("nop", "5b", 0, 1),   
    # PUSHn, DUPn, and SWAPn are added programatically later
...
# Add DUPn and SWAPn opcodes
for opc in range(1,17):
    opcodes.append(Opcode("dup" + str(opc), hex(0x80+opc-1)[2:], opc, 3))
    opcodes.append(Opcode("swap" + str(opc), hex(0x90+opc-1)[2:], opc+1, 3))    
...
def test_underflow(fork):
    ...
    for opcode in opcodes:
        ...
        pre[good_addr] = Account(
            ...
            code="0x" + "6000"*opcode.inps + opcode.code + "00"
        )
...
def test_gas_cost(fork):
    ...
    for opcode in opcodes:
        ...
        post = {
            "0x1000000000000000000000000000000000000000": Account(
                storage={
                    0x00: 2625+3*opcode.inps+opcode.gas
                },
            ),
        }
