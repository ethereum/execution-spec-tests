...
def is_cancun(fork: str) -> bool:
    """
    Returns `True` if `fork` is Cancun-compatible, `False` otherwise.
    """
    fork_lower = fork.lower()
    if fork_lower not in forks:
        return False
    ...
    i = forks.index(fork_lower)
    return i >= forks.index("shardingfork")
...
def must_contain_excess_data_gas(fork: str) -> bool:
    """
    Returns `True` if the environment is expected to have excess data gas
    """
    return is_cancun(fork)
...
def set_fork_requirements(env: Environment, fork: str) -> Environment:
    ...
    if (
        must_contain_excess_data_gas(fork)
