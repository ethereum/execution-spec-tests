...
def analyze_pre_alloc_file(file_path: Path, verbose: int = 0) -> Dict:
    """Analyze pre-allocation file and return statistics."""
    ...
    split_test_functions: Dict[str, Dict] = defaultdict(lambda: {"groups": 0, "forks": set()})
    ...
    split_functions = {}
    for func, data in split_test_functions.items():
        if data["groups"] > 1:
            fork_count = len(data["forks"])
            groups_per_fork = data["groups"] / fork_count if fork_count > 0 else data["groups"]
...
@click.argument(
    ...
    default="fixtures/blockchain_test_engine_reorg/pre_alloc.json",
)
@click.option(
    "--verbose",
    "-v",
...
def main(pre_alloc_file: Path, verbose: int):
    """
    Display statistics about shared pre-allocation groups.

    This script analyzes a pre_alloc.json file generated by the test framework's
    ...
    try:
        stats = analyze_pre_alloc_file(pre_alloc_file, verbose=verbose)
        display_stats(stats, console, verbose=verbose)
    except FileNotFoundError:
        console.print(f"[red]Error: File not found: {pre_alloc_file}[/red]")
        raise click.Abort() from None
    except json.JSONDecodeError as e:
        console.print(f"[red]Error: Invalid JSON in {pre_alloc_file}: {e}[/red]")
        raise click.Abort() from None
    except Exception as e:
