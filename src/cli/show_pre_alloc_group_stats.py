...
def calculate_size_distribution(
    ...
    Returns: - Group count distribution: [(range_label, group_count), ...] -
    Test count distribution: [(range_label, test_count, cumulative_remaining,
    group_count), ...]
    """
    if not test_counts:
        return [], []
    ...
    for min_val, max_val, label in bins:
        ...
        if group_count > 0:
            ...
            test_distribution.append((label, tests_in_bin, 0, group_count))  # Added
            # group_count

    # Calculate cumulative values For the table sorted from largest to
    # smallest: - Row N shows: if we exclude groups of size N and smaller, what
    # % of tests remain? - Row N shows: if we include groups of size N and
...
def display_stats(stats: Dict, console: Console, verbose: int = 0):
    ...
    if verbose >= 1:
        ...
        sorted_modules = sorted(
            ...
            key=lambda x: (-x[1]["groups"], -x[1]["tests"]),  # Secondary sort
            # by tests
        )
...
def main(pre_alloc_folder: Path, verbose: int):
    ...
    - Total number of groups, tests, and accounts - Number of tests and
    accounts per group (tabulated) - Number of groups and tests per fork
    (tabulated) - Number of groups and tests per test module (tabulated)
