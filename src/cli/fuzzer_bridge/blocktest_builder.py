...
class BlocktestBuilder:
    ...
    def parse_fuzzer_output(self, fuzzer_data: Dict[str, Any]) -> tuple:
        ...
        # Parse transactions
        transactions = []
        for tx_data in fuzzer_data.get("transactions", []):
            # Handle 'from' field - convert to 'sender'
            sender = Address(tx_data.get("from", tx_data.get("sender")))

            # Get private key for this sender if available
        ...
        # Parse environment
        env_data = fuzzer_data.get("env", {})
        env = Environment(
            fee_recipient=Address(env_data.get("currentCoinbase")),
            difficulty=0,  # Difficulty is ignored post-merge
        ...
        # Parse chain ID
        chain_id = (
            int(fuzzer_data.get("chainId", fuzzer_data.get("chainid", "1")), 16)
            if isinstance(fuzzer_data.get("chainId", fuzzer_data.get("chainid", "1")), str)
            else fuzzer_data.get("chainId", fuzzer_data.get("chainid", 1))
        )
    ...
    def build_blocktest(self, fuzzer_output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            fuzzer_output: Dictionary containing pre-state, transactions, environment, etc.

        Returns:
            Valid blocktest fixture as a dictionary.
        ...
        block = Block(txs=transactions, **block1_env)

        # Create blockchain test
        test = BlockchainTest(
            pre=pre,
            blocks=[block],
            post={},  # Will be calculated during fill
            genesis_environment=env,
            chain_id=chain_id,
        )

        # Generate fixture using the transition tool
        ...
        # Return as dictionary with proper serialization
...
def build_blocktest_from_fuzzer(
    ...
    """Convenience function to build blocktest from fuzzer output."""
