            ...
            with total_payload_timing.time(f"Step {i + 1} ({step.step_type})") as payload_timing:
                if isinstance(step, FixtureSendTransactionWithPost):
                    ...
                    with payload_timing.time(
                        ...
                        if step.sorted_transactions is not None:
                            assert (
                                new_payload.execution_payload.transactions
                                == step.sorted_transactions
                            ), (
                                f"unexpected sorted transactions: "
                                f"{new_payload.execution_payload.transactions}"
                            )
                        elif step.unsorted_transactions is not None:
                            assert set(new_payload.execution_payload.transactions) == set(
                                step.unsorted_transactions
                            ), (
                                f"unexpected unsorted transactions: "
                                f"{new_payload.execution_payload.transactions}"
                            )
                        elif step.mutually_exclusive_transactions is not None:
                            assert (
                                len(
                                    set(new_payload.execution_payload.transactions)
                                    & set(step.mutually_exclusive_transactions)
                                )
                                == 1
                            ), (
                                f"unexpected mutually exclusive transactions: "
                                f"{new_payload.execution_payload.transactions}"
                            )
                    ...
                    with payload_timing.time(
                        ...
                        assert new_payload_response.status == (
                            PayloadStatusEnum.VALID
