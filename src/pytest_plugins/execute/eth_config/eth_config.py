...
def all_rpc_endpoints(config) -> Dict[str, List[EthRPC]]:
    ...
    # url_dict looks like this: { 'besu': [<EthRPC that holds url for
    # grandine+besu>, <EthRPC that holds url for lighthouse+besu>, ..],  #
    # noqa: E501 'erigon':  ... ... }
    return url_dict
...
def pytest_generate_tests(metafunc: pytest.Metafunc):
    ...
    if metafunc.definition.name == "test_eth_config_majority":
        if len(all_rpc_endpoints_dict) < 2:
            ...
            # client, so no majority comparison  # noqa: E501
            logger.info(
