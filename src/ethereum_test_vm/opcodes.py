...
class Opcodes(Opcode, Enum):
    ...
    Inputs ---- - a: first integer value to add - b: second integer value to
    add

    Outputs ---- - c: integer result of the addition modulo 2**256
    ...
    Inputs ---- - a: first integer value to multiply - b: second integer value
    to multiply

    Outputs ---- - c: integer result of the multiplication modulo 2**256
    ...
    Inputs ---- - a: first integer value - b: second integer value

    Outputs ---- - c: integer result of the subtraction modulo 2**256
    ...
    Inputs ---- - a: numerator - b: denominator (must be non-zero)

    Outputs ---- - c: integer result of the division
    ...
    Description ---- Signed division operation

    Inputs ---- - a: signed numerator - b: signed denominator

    Outputs ---- - c: signed integer result of the division. If the denominator
    ...
    Inputs ---- - a: integer numerator - b: integer denominator

    Outputs ---- - a % b: integer result of the integer modulo. If the
    denominator is 0, the result will be 0
    ...
    Outputs ---- - a % b: integer result of the signed integer modulo. If the
    denominator is 0, the result will be 0
    ...
    Inputs ---- - a: first integer value - b: second integer value - c: integer
    denominator

    Outputs ---- - (a + b) % N: integer result of the addition followed by a
    modulo. If the denominator is 0, the result will be 0
    ...
    Inputs ---- - a: first integer value to multiply - b: second integer value
    to multiply - N: integer denominator

    Outputs ---- - (a * b) % N: integer result of the multiplication followed
    by a modulo. If the denominator is 0, the result will be 0
    ...
    Inputs ---- - a: integer base - exponent: integer exponent

    Outputs ---- - a ** exponent: integer result of the exponential operation
    modulo 2**256

    Fork ---- Frontier

    Gas ---- - static_gas = 10 - dynamic_gas = 50 * exponent_byte_size
    ...
    Description ---- Sign extension operation

    Inputs ---- - b: size in byte - 1 of the integer to sign extend - x:
    integer value to sign extend

    Outputs ---- - y: integer result of the sign extend
    ...
    Inputs ---- - a: left side integer value - b: right side integer value

    Outputs ---- - a < b: 1 if the left side is smaller, 0 otherwise
    ...
    Inputs ---- - a: left side integer - b: right side integer

    Outputs ---- - a > b: 1 if the left side is bigger, 0 otherwise
    ...
    Inputs ---- - a: left side signed integer - b: right side signed integer
    ...
    Outputs ---- - a == b: 1 if the left side is equal to the right side, 0
    otherwise
    ...
    Outputs ---- - a == 0: 1 if a is 0, 0 otherwise
    ...
    Inputs ---- - a: first binary value - b: second binary value

    Outputs ---- - a & b: the bitwise AND result
    ...
    Outputs ---- - a | b: the bitwise OR result
    ...
    Outputs ---- - a ^ b: the bitwise XOR result
    ...
    Outputs ---- - ~a: the bitwise NOT result
    ...
    Description ---- Extract a byte from the given position in the value

    Inputs ---- - i: byte offset starting from the most significant byte - x:
    32-byte value

    Outputs ---- - y: the indicated byte at the least significant position. If
    the byte offset is out of range, the result is 0
    ...
    Inputs ---- - shift: number of bits to shift to the left - value: 32 bytes
    to shift

    Outputs ---- - value << shift: the shifted value. If shift is bigger than
    255, returns 0
    ...
    Inputs ---- - shift: number of bits to shift to the right. - value: 32
    bytes to shift
    ...
    Inputs ---- - shift: number of bits to shift to the right - value: integer
    to shift

    Outputs ---- - value >> shift: the shifted value
    ...
    Description ---- Counts leading zeros (bitwise).

    Inputs ---- - value: integer to count zeros on

    Outputs ---- - zeros: leading zero bits
    ...
    Inputs ---- - offset: byte offset in the memory - size: byte size to read
    in the memory

    Outputs ---- - hash: Keccak-256 hash of the given data in memory

    Fork ---- Frontier

    Gas ---- - minimum_word_size = (size + 31) / 32 - static_gas = 30 -
    ...
    Outputs ---- - address: the 20-byte address of the current account
    ...
    Inputs ---- - address: 20-byte address of the account to check

    Outputs ---- - balance: balance of the given account in wei. Returns 0 if
    the account doesn't exist

    Fork ---- Frontier

    Gas ---- - static_gas = 0 - dynamic_gas = 100 if warm_address, 2600 if
    cold_address
    ...
    Outputs ---- - address: the 20-byte address of the sender of the
    transaction. It can only be an account without code
    ...
    Outputs ---- - address: the 20-byte address of the caller account. This is
    the account that did the last call (except delegate call)
    ...
    Outputs ---- - value: the value of the current call in wei
    ...
    Inputs ---- - offset: byte offset in the calldata

    Outputs ---- - data[offset]: 32-byte value starting from the given offset
    of the calldata. All bytes after the end of the calldata are set to 0
    ...
    Outputs ---- - size: byte size of the calldata
    ...
    Inputs ---- - dest_offset: byte offset in the memory where the result will
    be copied - offset: byte offset in the calldata to copy - size: byte size
    to copy
    ...
    Gas ---- - minimum_word_size = (size + 31) / 32 - static_gas = 3 -
    ...
    Outputs ---- - size: byte size of the code
    ...
    Inputs ---- - dest_offset: byte offset in the memory where the result will
    be copied. - offset: byte offset in the code to copy. - size: byte size to
    copy
    ...
    Outputs ---- - price: gas price in wei per gas
    ...
    Inputs ---- - address: 20-byte address of the contract to query
    ...
    offset: byte offset in the code to copy - size: byte size to copy

    Outputs ---- - None

    Fork ---- Frontier

    Gas ---- - minimum_word_size = (size + 31) / 32 - static_gas = 0 -
    dynamic_gas = 3 * minimum_word_size + memory_expansion_cost +
    address_access_cost
    ...
    Description ---- Get size of output data from the previous call from the
    current environment

    Outputs ---- - size: byte size of the return data from the last executed
    sub context
    ...
    Inputs ---- - dest_offset: byte offset in the memory where the result will
    be copied - offset: byte offset in the return data from the last executed
    sub context to copy - size: byte size to copy
    ...
    Inputs ---- - address: 20-byte address of the account

    Outputs ---- - hash: hash of the chosen account's code, the empty hash
    (0xc5d24601...) if the account has no code, or 0 if the account does not
    ...
    Inputs ---- - blockNumber: block number to get the hash from. Valid range
    is the last 256 blocks (not including the current one). Current block
    number can be queried with NUMBER

    Outputs ---- - hash: hash of the chosen block, or 0 if the block number is
    ...
    Outputs ---- - address: miner's 20-byte address
    ...
    Outputs ---- - timestamp: unix timestamp of the current block
    ...
    Outputs ---- - blockNumber: current block number
    ...
    Outputs ---- - prevRandao: previous block's RANDAO mix
    ...
    Outputs ---- - chainId: chain id of the network
    ...
    Outputs ---- - balance: balance of the current account in wei
    ...
    Description ---- Returns the versioned hash of a single blob contained in
    the type-3 transaction

    Inputs ---- - index: index of the blob

    Outputs ---- - versionedHash: versioned hash of the blob
    ...
    Description ---- Returns the value of the blob base fee of the block it is
    executing in

    Inputs ---- - None

    Outputs ---- - baseFeePerBlobGas: base fee for the blob gas in wei
    ...
    Inputs ---- - offset: offset in the memory in bytes

    Outputs ---- - value: the 32 bytes in memory starting at that offset. If it
    goes beyond its current size (see MSIZE), writes 0s

    Fork ---- Frontier

    Gas ---- - static_gas = 3 - dynamic_gas = memory_expansion_cost
    ...
    Inputs ---- - offset: offset in the memory in bytes - value: 32-byte value
    ...
    Inputs ---- - offset: offset in the memory in bytes - value: 1-byte value
    to write in the memory (the least significant byte of the 32-byte stack
    value)
    ...
    Inputs ---- - key: 32-byte key in storage

    Outputs ---- - value: 32-byte value corresponding to that key. 0 if that
    ...
    Inputs ---- - key: 32-byte key in storage - value: 32-byte value to store
    ...
    if value == current_value if key is warm base_dynamic_gas = 100 else
    base_dynamic_gas = 100 else if current_value == original_value if
    original_value == 0 base_dynamic_gas = 20000 else base_dynamic_gas = 2900
    else base_dynamic_gas = 100

    if key is cold: base_dynamic_gas += 2100 ```
    ...
    Inputs ---- - pc: byte offset in the deployed code where execution will
    continue from. Must be a JUMPDEST instruction
    ...
    Inputs ---- - pc: byte offset in the deployed code where execution will
    continue from. Must be a JUMPDEST instruction - condition: the program
    counter will be altered with the new value only if this value is different
    from 0. Otherwise, the program counter is simply incremented and the next
    ...
    Description ---- Get the value of the program counter prior to the
    increment corresponding to this instruction

    Inputs ---- - None

    Outputs ---- - counter: PC of this instruction in the current program.
    ...
    Outputs ---- - size: current memory size in bytes (higher offset accessed
    until now + 1)
    ...
    Description ---- Get the amount of available gas, including the
    corresponding reduction for the cost of this instruction

    Inputs ---- - None

    Outputs ---- - gas: remaining gas (after this instruction)
    ...
    Description ---- Synonym for JUMPDEST. Performs no operation.
    ...
    Inputs ---- - key: 32-byte key in transient storage
    ...
    Source:
    [eips.ethereum.org/EIPS/eip-1153](https://eips.ethereum.org/EIPS/eip-1153)
    ...
    Inputs ---- - key: 32-byte key in transient storage - value: 32-byte value
    to store
    ...
    Description ---- Copies areas in memory
    ...
    Source:
    [eips.ethereum.org/EIPS/eip-5656](https://eips.ethereum.org/EIPS/eip-5656)
    ...
    Outputs ---- - value: pushed value, equal to 0
    ...
    Outputs ---- - value: pushed value, aligned to the right (put in the lowest
    significant bytes)
    ...
    Inputs ---- - value: value to duplicate

    Outputs ---- - value: duplicated value - value: original value
    ...
    Inputs ---- - v1: ignored value - v2: value to duplicate

    Outputs ---- - v2: duplicated value - v1: ignored value - v2: original
    value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - v3: value to
    duplicate

    Outputs ---- - v3: duplicated value - v1: ignored value - v2: ignored value
    - v3: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - v3: ignored value -
    v4: value to duplicate

    Outputs ---- - v4: duplicated value - v1: ignored value - v2: ignored value
    - v3: ignored value - v4: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - v3: ignored value -
    v4: ignored value - v5: value to duplicate

    Outputs ---- - v5: duplicated value - v1: ignored value - v2: ignored value
    - v3: ignored value - v4: ignored value - v5: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v5: ignored
    value - v6: value to duplicate

    Outputs ---- - v6: duplicated value - v1: ignored value - v2: ignored value
    - ... - v5: ignored value - v6: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v6: ignored
    value - v7: value to duplicate

    Outputs ---- - v7: duplicated value - v1: ignored value - v2: ignored value
    - ... - v6: ignored value - v7: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v7: ignored
    value - v8: value to duplicate

    Outputs ---- - v8: duplicated value - v1: ignored value - v2: ignored value
    - ... - v7: ignored value - v8: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v8: ignored
    value - v9: value to duplicate

    Outputs ---- - v9: duplicated value - v1: ignored value - v2: ignored value
    - ... - v8: ignored value - v9: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v9: ignored
    value - v10: value to duplicate

    Outputs ---- - v10: duplicated value - v1: ignored value - v2: ignored
    value - ... - v9: ignored value - v10: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v10: ignored
    value - v11: value to duplicate

    Outputs ---- - v11: duplicated value - v1: ignored value - v2: ignored
    value - ... - v10: ignored value - v11: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v11: ignored
    value - v12: value to duplicate

    Outputs ---- - v12: duplicated value - v1: ignored value - v2: ignored
    value - ... - v11: ignored value - v12: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v12: ignored
    value - v13: value to duplicate

    Outputs ---- - v13: duplicated value - v1: ignored value - v2: ignored
    value - ... - v12: ignored value - v13: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v13: ignored
    value - v14: value to duplicate

    Outputs ---- - v14: duplicated value - v1: ignored value - v2: ignored
    value - ... - v13: ignored value - v14: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v14: ignored
    value - v15: value to duplicate

    Outputs ---- - v15: duplicated value - v1: ignored value - v2: ignored
    value - ... - v14: ignored value - v15: original value
    ...
    Inputs ---- - v1: ignored value - v2: ignored value - ... - v15: ignored
    value - v16: value to duplicate

    Outputs ---- - v16: duplicated value - v1: ignored value - v2: ignored
    value - ... - v15: ignored value - v16: original value
    ...
    Description ---- Exchange the top stack item with the second stack item.

    Inputs ---- - v1: value to swap - v2: value to swap

    Outputs ---- - v1: swapped value - v2: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - v3: value to swap

    Outputs ---- - v3: swapped value - v2: ignored value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - v3: ignored value -
    v4: value to swap

    Outputs ---- - v4: swapped value - v2: ignored value - v3: ignored value -
    v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v4: ignored
    value - v5: value to swap

    Outputs ---- - v5: swapped value - v2: ignored value - ... - v4: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v5: ignored
    value - v6: value to swap

    Outputs ---- - v6: swapped value - v2: ignored value - ... - v5: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v6: ignored
    value - v7: value to swap

    Outputs ---- - v7: swapped value - v2: ignored value - ... - v6: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v7: ignored
    value - v8: value to swap

    Outputs ---- - v8: swapped value - v2: ignored value - ... - v7: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v8: ignored
    value - v9: value to swap

    Outputs ---- - v9: swapped value - v2: ignored value - ... - v8: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v9: ignored
    value - v10: value to swap

    Outputs ---- - v10: swapped value - v2: ignored value - ... - v9: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v10: ignored
    value - v11: value to swap

    Outputs ---- - v11: swapped value - v2: ignored value - ... - v10: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v11: ignored
    value - v12: value to swap

    Outputs ---- - v12: swapped value - v2: ignored value - ... - v11: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v12: ignored
    value - v13: value to swap

    Outputs ---- - v13: swapped value - v2: ignored value - ... - v12: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v13: ignored
    value - v14: value to swap

    Outputs ---- - v14: swapped value - v2: ignored value - ... - v13: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v14: ignored
    value - v15: value to swap

    Outputs ---- - v15: swapped value - v2: ignored value - ... - v14: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v15: ignored
    value - v16: value to swap

    Outputs ---- - v16: swapped value - v2: ignored value - ... - v15: ignored
    value - v1: swapped value
    ...
    Inputs ---- - v1: value to swap - v2: ignored value - ... - v16: ignored
    value - v17: value to swap

    Outputs ---- - v17: swapped value - v2: ignored value - ... - v16: ignored
    value - v1: swapped value
    ...
    Gas ---- - static_gas = 375 - dynamic_gas = 375 * topic_count + 8 * size +
    ...
    Inputs ---- - offset: byte offset in the memory in bytes - size: byte size
    to copy - topic_1: 32-byte value - topic_2: 32-byte value
    ...
    Inputs ---- - offset: byte offset in the memory in bytes - size: byte size
    to copy - topic_1: 32-byte value - topic_2: 32-byte value - topic_3:
    32-byte value
    ...
    Inputs ---- - offset: byte offset in the memory in bytes - size: byte size
    to copy - topic_1: 32-byte value - topic_2: 32-byte value - topic_3:
    32-byte value - topic_4: 32-byte value
    ...
    Source:
    [eips.ethereum.org/EIPS/eip-4200](https://eips.ethereum.org/EIPS/eip-4200)
    ...
    Description ---- Reads 32 bytes of data at offset onto the stack

    Inputs ---- - offset: offset within the data section to start copying
    ...
    Source:
    [eips.ethereum.org/EIPS/eip-7480](https://eips.ethereum.org/EIPS/eip-7480)
    ...
    Immediates ---- 2 bytes forming a UInt16, which is the offset into the data
    section.
    ...
    Description ---- Returns the size of the data section

    Inputs ----

    Outputs ---- The size of the data section. If there is no data section,
    returns 0.
    ...
    Description ---- Copies data from the data section into call frame memory

    Inputs ---- - dest_offset: The offset within the memory section to start
    copying to - offset: The offset within the data section to start copying
    from - size: The number of bytes to copy
    ...
    Description ---- Relative jump with variable offset.

    When calling this opcode to generate bytecode, the first argument is used
    to format the data portion of the opcode, and it can be either of two
    types: - A bytes type, and in this instance the bytes are used verbatim as
    the data portion. - An integer iterable, list or tuple or any other
    iterable, where each element is a jump offset.
    ...
    - deduct 5 gas - read uint16 operand idx - if 1024 < len(stack) +
    types[idx].max_stack_height - types[idx].inputs, execution results in an
    exceptional halt - if 1024 <= len(return_stack), execution results in an
    exceptional halt - push new element to return_stack (current_code_idx,
    pc+3) - update current_code_idx to idx and set pc to 0

    Inputs ---- Any: The inputs are not checked because we cannot know how many
    inputs the callee function/section requires

    Outputs ---- Any: The outputs are variable because we cannot know how many
    outputs the callee function/section produces
    ...
    Source:
    [ipsilon/eof/blob/main/spec/eof.md](https://github.com/ipsilon/eof/blob/main/sp
    ec/eof.md)
    ...
    - deduct 5 gas - read uint16 operand idx - if 1024 < len(stack) +
    types[idx].max_stack_height - types[idx].inputs, execution results in an
    exceptional halt - set current_code_idx to idx - set pc = 0
    ...
    - deduct 3 gas - read uint8 operand imm - n = imm + 1 - n‘th (1-based)
    stack item is duplicated at the top of the stack - Stack validation:
    stack_height >= n
    ...
    - deduct 3 gas - read uint8 operand imm - n = imm + 1 - n + 1th stack item
    is swapped with the top stack item (1-based). - Stack validation:
    stack_height >= n + 1
    ...
    Description ---- Exchanges two stack positions.  Two nybbles, n is high 4
    bits + 1, then  m is 4 low bits + 1. Exchanges the n+1'th item with the n +
    m + 1 item.

    Inputs x and y when the opcode is used as `EXCHANGE[x, y]`, are equal to: -
    x = n + 1 - y = n + m + 1 Which each equals to 1-based stack positions
    swapped.

    Inputs ---- n + m + 1, or ((imm >> 4) + (imm &0x0F) + 3) from the raw
    immediate,

    Outputs ---- n + m + 1, or ((imm >> 4) + (imm &0x0F) + 3) from the raw
    immediate,
    ...
    Description ---- Create a new contract with the given code

    Inputs ---- - value: value in wei to send to the new account - offset: byte
    offset in the memory in bytes, the initialization code for the new account
    - size: byte size to copy (size of the initialization code)

    Outputs ---- - address: the address of the deployed contract, 0 if the
    deployment failed

    Fork ---- Frontier

    Gas ---- ``` minimum_word_size = (size + 31) / 32 init_code_cost = 2 *
    minimum_word_size code_deposit_cost = 200 * deployed_code_size

    static_gas = 32000 dynamic_gas = init_code_cost + memory_expansion_cost +
    ...
    Inputs ---- - gas: amount of gas to send to the sub context to execute. The
    gas that is not used by the sub context is returned to this one - address:
    the account which context to execute - value: value in wei to send to the
    account - args_offset: byte offset in the memory in bytes, the calldata of
    the sub context - args_size: byte size to copy (size of the calldata) -
    ret_offset: byte offset in the memory in bytes, where to store the return
    data of the sub context - ret_size: byte size to copy (size of the return
    data)

    Outputs ---- - success: return 0 if the sub context reverted, 1 otherwise

    Fork ---- Frontier

    Gas ---- ``` static_gas = 0 dynamic_gas = memory_expansion_cost +
    code_execution_cost + address_access_cost + positive_value_cost +
    ...
    Description ---- Message-call into this account with an alternative
    account's code. Executes code starting at the address to which the call is
    made.

    Inputs ---- - gas: amount of gas to send to the sub context to execute. The
    gas that is not used by the sub context is returned to this one - address:
    the account which code to execute - value: value in wei to send to the
    account - args_offset: byte offset in the memory in bytes, the calldata of
    the sub context - args_size: byte size to copy (size of the calldata) -
    ret_offset: byte offset in the memory in bytes, where to store the return
    data of the sub context - ret_size: byte size to copy (size of the return
    ...
    Inputs ---- - offset: byte offset in the memory in bytes, to copy what will
    be the return data of this context - size: byte size to copy (size of the
    return data)
    ...
    Inputs ---- - gas: amount of gas to send to the sub context to execute. The
    gas that is not used by the sub context is returned to this one - address:
    the account which code to execute - args_offset: byte offset in the memory
    in bytes, the calldata of the sub context - args_size: byte size to copy
    (size of the calldata) - ret_offset: byte offset in the memory in bytes,
    where to store the return data of the sub context - ret_size: byte size to
    copy (size of the return data)
    ...
    Gas ---- - static_gas = 0 - dynamic_gas = memory_expansion_cost +
    code_execution_cost + address_access_cost
    ...
    Inputs ---- - value: value in wei to send to the new account - offset: byte
    offset in the memory in bytes, the initialization code of the new account -
    size: byte size to copy (size of the initialization code) - salt: 32-byte
    value used to create the new account at a deterministic address
    ...
    Gas ---- ``` minimum_word_size = (size + 31) / 32 init_code_cost = 2 *
    minimum_word_size hash_cost = 6 * minimum_word_size code_deposit_cost = 200
    * deployed_code_size

    static_gas = 32000 dynamic_gas = init_code_cost + hash_cost +
    memory_expansion_cost + deployment_code_execution_cost + code_deposit_cost
    ```
    ...
    Inputs ---- - address: the account which context to execute - args_offset:
    byte offset in the memory in bytes, the calldata of the sub context -
    args_size: byte size to copy (size of the calldata) - value: value in wei
    to send to the account

    Outputs ---- - success: - `0` if the call was successful. - `1` if the call
    has reverted (also can be pushed earlier in a light failure scenario). -
    ...
    Inputs ---- - gas: amount of gas to send to the sub context to execute. The
    gas that is not used by the sub context is returned to this one - address:
    the account which context to execute - args_offset: byte offset in the
    memory in bytes, the calldata of the sub context - args_size: byte size to
    copy (size of the calldata) - ret_offset: byte offset in the memory in
    bytes, where to store the return data of the sub context - ret_size: byte
    size to copy (size of the return data)
    ...
    Description ---- Copy 32 bytes from returndata at offset onto the stack

    Inputs ---- - offset: byte offset in the return data from the last executed
    sub context to copy
    ...
    Description ---- Halt execution reverting state changes but returning data
    and remaining gas

    Inputs ---- - offset: byte offset in the memory in bytes. The return data
    of the calling context - size: byte size to copy (size of the return data)
    ...
    Gas ---- All the remaining gas in this context is consumed
    ...
    Description ---- Halt execution and register the account for later deletion

    Inputs ---- - address: account to send the current balance to
...
class Macros(Macro, Enum):
    ...
    Inputs ---- - None. Any input arguments are ignored.

    Outputs ---- - None

    Fork ---- Frontier

    Gas ---- `SHA3(0, 100000000000)` results in 19073514453125027 gas used and
    an OOG exception.

    Note: If a value > `100000000000` is used as second argument, the resulting
    geth trace reports gas `30` and an OOG exception. `SHA3(0, SUB(0, 1))`
    causes a gas > u64 exception and an OOG exception.

    Bytecode ---- SHA3(0, 100000000000)
    ...
    Inputs ---- - data: The data to store in memory. Can be an integer or
    bytes. - offset: The offset in memory to store the data.
